...
- **pymediainfo**: Video metadata extraction (duration, codec, resolution) - PRIMARY
  - Fallback to **ffprobe** (subprocess) if pymediainfo unavailable
  - pymediainfo: Easier Python integration, ~3.5x faster for single files
  - ffprobe: Better automation, wider system compatibility (FFmpeg usually pre-installed)
...
- **concurrent.futures**: Parallel hashing (saved for optimization phase, not MVP)
...
### Stage 3A: Internal Deduplication (Within Folders)

**Goal**: Remove all duplicate files within each folder (input and output) independently.

**Process** (applied to BOTH input and output folders):
1. Scan all files in folder (recursive)
...
7. Update folder cache

**Benefits**:
- Cleans up both input AND output folders independently
- Reduces dataset size for Stage 3B
- Finds duplicates that wouldn't be caught in cross-comparison
- Output folder deduplication catches duplicates introduced by previous operations
...
|-----------|-------|----------|----------|
| **xxHash** | **10-20 GB/s** | Non-cryptographic | **File deduplication (ONLY CHOICE)** |
| SHA-1 | 500-700 MB/s | Deprecated (crypto) | NOT USED |
| SHA-256 | 300-500 MB/s | Secure | NOT USED |
| MD5 | 800-1000 MB/s | Known collisions | NOT USED |
...
**Note**: xxHash is the ONLY supported hash algorithm for Stage 3. Alternative algorithms (SHA-256, SHA-1, MD5) are NOT implemented to keep the codebase focused and optimized.
...
```yaml
# .file_organizer.yaml (in execution directory)
...
duplicate_detection:
  ...
  # Hash algorithm: xxhash (ONLY OPTION - no alternatives)
  ...
  # Cache settings
  cache_type: sqlite                # sqlite (only option)
  cache_location: ./.file_organizer_cache/hashes.db  # Relative to execution directory
  ...
  video_metadata_tool: pymediainfo  # Primary: pymediainfo, Fallback: ffprobe
...
```bash
# Disable sampling for full accuracy (hash entire files)
--no-sampling

# Force output folder comparison even if not in config
--compare-output

# Override video metadata tool
--video-tool ffprobe  # Use ffprobe instead of pymediainfo
```
